...
# ==========================================
# ฟังก์ชันสร้าง HTML และ Component ต่างๆ
# ==========================================
...
def generate_fraction_html(num, den, color="#000", is_bold=True):
    w = "bold" if is_bold else "normal"
    return f"""<div style="display: inline-flex; flex-direction: column; align-items: center; vertical-align: middle; margin: 0 5px; font-family: 'Sarabun', sans-serif;"><span style="font-size: 20px; font-weight: {w}; border-bottom: 2px solid {color}; padding: 0 4px; line-height: 1.1; color: {color};">{num}</span><span style="font-size: 20px; font-weight: {w}; padding: 0 4px; line-height: 1.1; color: {color};">{den}</span></div>"""
...
def generate_questions_logic(grade, main_t, sub_t, num_q):
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # =========================================================
            # ฐานข้อมูลโจทย์ 50 หัวข้อย่อย (King Math Edition) ครบถ้วน 100%
            # =========================================================
            if actual_sub_t == "การคูณ (แบบตั้งหลัก)":
                ...
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                <b>ขั้นที่ 1:</b> ตัวเลขหน้าเครื่องหมาย : บ่งบอกถึงหน่วย 'นาฬิกา' คือ <b>{h_24}</b><br>
                <b>ขั้นที่ 2:</b> ตัวเลขหลังเครื่องหมาย : บ่งบอกถึงหน่วย 'นาที' คือ <b>{m}</b><br>
                <b>ขั้นที่ 3:</b> นำมาอ่านรวมกัน จะได้ <b>{h_24} นาฬิกา {m} นาที</b><br>
                ...
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                <b>ขั้นที่ 1:</b> เข็มชี้เลยเลข {kg} แปลว่ามีน้ำหนักหลักคือ <b>{kg} กิโลกรัม</b><br>
                <b>ขั้นที่ 2:</b> แปลงหน่วยขีดเป็นหน่วยกรัม (1 ขีด = 100 กรัมเสมอ)<br>
                👉 นำ {kheed} ขีด × 100 = <b>{g} กรัม</b><br>
                <b>ขั้นที่ 3:</b> นำน้ำหนักมารวมกัน ➔ <b>{kg} กิโลกรัม {g} กรัม</b><br>
                ...
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                <b>ขั้นที่ 1: ตรวจสอบข้อกำหนดของแผนภูมิ</b><br>
                👉 รูป 1 รูป ไม่ได้แปลว่า 1 ผล แต่โจทย์กำหนดให้ 1 รูป = <b>{pic_val} ผล</b><br>
                <b>ขั้นที่ 2: นับจำนวนรูปในตาราง</b><br>
                👉 ในตารางมีรูปวาดทั้งหมด <b>{count} รูป</b><br>
                <b>ขั้นที่ 3: คำนวณหาจำนวนที่แท้จริง</b><br>
                👉 นำจำนวนรูปภาพ × ข้อกำหนด: {count} × {pic_val} = <b>{count * pic_val} ผล</b><br>
            ...
            elif actual_sub_t in ["การหาความยาวรอบรูปสี่เหลี่ยมมุมฉาก", "การหาพื้นที่รูปสี่เหลี่ยมมุมฉาก"]:
...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังประมวลผลคำอธิบายและวาดภาพเฉลยแบบ Step-by-Step พร้อมเศษส่วนแนวตั้ง..."):
        ...
        qs = generate_questions_logic(selected_grade, selected_main, selected_sub, num_input, False) # Using standard logic
        ...
        filename_base = f"TMC_{selected_grade}_{selected_sub}"

...
FRUIT_EMOJIS = {"แอปเปิล": "🍎", "ส้ม": "🍊", "สตรอว์เบอร์รี": "🍓", "กล้วย": "🍌", "มะม่วง": "🥭", "แตงโม": "🍉"}
...
def draw_clock_svg(h_24, m):
    ...
    # วาดเข็มชั่วโมง (สีแดง) และ เข็มนาที (สีฟ้า)
...
def draw_scale_svg(kg, kheed, max_kg=5):
    ...
    # เข็มสเกลทั้งหมด 50 ขีด = 360 องศา -> 1 ขีด = 7.2 องศา (เริ่ม 0 ที่ -90 องศา)
...
def draw_pictogram_html(item, emoji, count, pic_val):
    icons = "".join([f"<span style='margin: 0 5px;'>{emoji}</span>"] * count)
    html = f"""
    <div style="border: 2px solid #34495e; border-radius: 10px; padding: 15px; width: 80%; margin: 15px auto; background-color: #fff;">
        <div style="text-align: center; background-color: #ecf0f1; padding: 10px; font-weight: bold; border-radius: 5px; font-size: 20px; border: 1px solid #bdc3c7;">
            จำนวน {item} ที่ขายได้
        </div>
        <div style="padding: 20px; font-size: 45px; text-align: center; display: flex; justify-content: center; flex-wrap: wrap;">
            {icons}
        </div>
        <div style="border-top: 2px dashed #bdc3c7; padding-top: 10px; text-align: center; font-size: 18px; color: #e74c3c; font-weight: bold;">
...
def generate_decimal_vertical_html(a, b, op, is_key=False):
    ...
    for i in range(max_len):
        ...
        if val and val != '.':
            ...
            if strike[i] and is_key: 
                td_content = f'<div style="position: relative;"><span style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #c0392b; font-weight: bold;">{mark}</span><span style="text-decoration: line-through; text-decoration-color: #c0392b; text-decoration-thickness: 2px;">{val}</span></div>'
            elif mark and is_key: 
                td_content = f'<div style="position: relative;"><span style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #c0392b; font-weight: bold;">{mark}</span><span>{val}</span></div>'
    ...
    if is_key: 
        ans_tds = "".join([f"<td style='width: 35px; text-align: center; color: #c0392b; font-weight: bold; height: 45px; vertical-align: bottom;'>{c.strip() if c.strip() else ('.' if c=='.' else '')}</td>" for c in str_ans])
    else: 
...
def generate_long_division_step_by_step_html(divisor, dividend, equation_html, is_key=False):
    ...
    ans_tds_list = [f'<td style="width: 35px; text-align: center; color: #c0392b; font-weight: bold; font-size: 38px;">{c.strip()}</td>' for c in ans_padded]
    ans_tds_list.append('<td style="width: 35px;"></td>') 
    ...
    for i, c in enumerate(div_str):
        ...
        if is_key and s0 and s0_start <= i <= s0['col_index']:
            ...
            if is_strik: 
                td_content = f'<div style="position: relative;"><span style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #c0392b; font-weight: bold;">{mark}</span><span style="text-decoration: line-through; text-decoration-color: #c0392b; text-decoration-thickness: 2px;">{c}</span></div>'
            elif mark: 
                td_content = f'<div style="position: relative;"><span style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #c0392b; font-weight: bold;">{mark}</span><span>{c}</span></div>'
        ...
        for i in range(div_len + 1):
            if i >= pad_len_rem and i <= step['col_index'] + (1 if not is_last_step else 0):
                ...
                if is_key and next_step and ns_start <= i <= next_step['col_index']:
                    ...
                    if is_strik: 
                        td_content = f'<div style="position: relative;"><span style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #c0392b; font-weight: bold;">{mark}</span><span style="text-decoration: line-through; text-decoration-color: #c0392b; text-decoration-thickness: 2px;">{char_val}</span></div>'
                    elif mark: 
                        td_content = f'<div style="position: relative;"><span style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); font-size: 20px; color: #c0392b; font-weight: bold;">{mark}</span><span>{char_val}</span></div>'
...
# ==========================================
# 2. ฐานข้อมูลหลักสูตร (Master Database) ครบถ้วน (ไม่มี King Math / TMC)
# ==========================================
curriculum_db = {
    "ป.1": {
...
# ==========================================
# 3. Logic & Dynamic Difficulty Scaling (ตรรกะฉบับสมบูรณ์ไม่มีตัดทิ้ง)
# ==========================================
def generate_questions_logic(grade, main_t, sub_t, num_q, is_challenge=False):
    ...
    items_shopping = ["โทรทัศน์", "ตู้เย็น", "เครื่องซักผ้า", "พัดลม", "เตารีด", "หม้อหุงข้าว", "โทรศัพท์มือถือ", "คอมพิวเตอร์", "เครื่องดูดฝุ่น", "แอร์"]
    items_fashion = ["รองเท้าผ้าใบ", "กระเป๋าเป้", "เสื้อแจ็คเก็ต", "กางเกงยีนส์", "นาฬิกาข้อมือ"]
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # =========================================================
            # ฐานข้อมูลโจทย์ทั้งหมด (ตรวจสอบชื่อหัวข้อให้ตรงกับ Database 100%)
            # =========================================================
            ...
            if actual_sub_t == "การบอกเวลาเป็นนาฬิกาและนาที":
                ...
                pic_val = random.choice([2, 5, 10])
                count = random.randint(3, 8)
                
                q = draw_pictogram_html(item, emoji, count, pic_val) + f"<br>จากแผนภูมิรูปภาพ มี{item}จริงๆ ทั้งหมดกี่ผล?"
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                <b>ขั้นที่ 1: ตรวจสอบข้อกำหนดของแผนภูมิที่ด้านล่าง</b><br>
                👉 รูป 1 รูป ไม่ได้แปลว่า 1 ผล แต่โจทย์กำหนดให้ 1 รูป = <b>{pic_val} ผล</b><br>
                <b>ขั้นที่ 2: นับจำนวนรูปภาพในตาราง</b><br>
                👉 ในตารางมีรูป {emoji} วาดไว้ทั้งหมด <b>{count} รูป</b><br>
                <b>ขั้นที่ 3: คำนวณหาจำนวนที่แท้จริง</b><br>
                👉 นำจำนวนรูปภาพ × ข้อกำหนดของ 1 รูป: {count} × {pic_val} = <b>{count * pic_val} ผล</b><br>
                <b>ตอบ: {count * pic_val} ผล</b></span>"""
            ...
            # ---------------- โหมดหลักสูตรปกติอื่นๆ (ครบถ้วนไม่มีตัด) ----------------
                ...
                col_name = cols[idx]
                q = f"มีรถ 4 คันวิ่งเรียงกันตามลำดับจากซ้ายไปขวา (คันซ้ายสุดคือคันที่ 1) ได้แก่ รถสี{cols[0]}, สี{cols[1]}, สี{cols[2]}, และ สี{cols[3]}<br>รถสี{col_name} วิ่งอยู่อันดับที่เท่าไร?"
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                ...
                👉 นับจำนวนช่องที่ถูกระบายสี จะได้ <b>{num} ช่อง</b><br>
            ...
            elif actual_sub_t == "การหาอัตราส่วนที่เท่ากัน":
                ...
                if is_front:
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    <b>ขั้นที่ 1: สังเกตความสัมพันธ์ของตัวเลขที่มีคู่ครบ</b><br>
                    👉 ลองสังเกตตัวเลขฝั่งหน้าของเครื่องหมาย : ทั้งสองข้าง คือ {a} ไปเป็น {a*m}<br>
                    <b>ขั้นที่ 2: หาตัวคูณ</b><br>
                    👉 จาก {a} กลายไปเป็น {a*m} แสดงว่ามันถูกนำไป <b>คูณด้วย {m}</b><br>
                    <b>ขั้นที่ 3: นำไปคูณอีกฝั่ง</b><br>
                    👉 หลักการของอัตราส่วนที่เท่ากันคือ ถ้าตัวหน้าคูณด้วยอะไร ตัวหลังก็ต้องคูณด้วยสิ่งนั้นด้วย!<br>
                    👉 นำตัวหลัง ({b}) ไปคูณด้วย {m} เช่นกัน ➔ {b} × {m} = <b>{b*m}</b><br>
                    <b>ตอบ: ตัวเลขในช่องว่างคือ {b*m}</b></span>"""
                else:
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    <b>ขั้นที่ 1: สังเกตความสัมพันธ์ของตัวเลขที่มีคู่ครบ</b><br>
                    👉 ลองสังเกตตัวเลขฝั่งหลังของเครื่องหมาย : ทั้งสองข้าง คือ {b} ไปเป็น {b*m}<br>
                    <b>ขั้นที่ 2: หาตัวคูณ</b><br>
                    👉 จาก {b} กลายไปเป็น {b*m} แสดงว่ามันถูกนำไป <b>คูณด้วย {m}</b><br>
                    <b>ขั้นที่ 3: นำไปคูณอีกฝั่ง</b><br>
                    👉 หลักการของอัตราส่วนที่เท่ากันคือ ถ้าตัวหลังคูณด้วยอะไร ตัวหน้าก็ต้องคูณด้วยสิ่งนั้นด้วย!<br>
                    👉 นำตัวหน้า ({a}) ไปคูณด้วย {m} เช่นกัน ➔ {a} × {m} = <b>{a*m}</b><br>
                    <b>ตอบ: ตัวเลขในช่องว่างคือ {a*m}</b></span>"""

            elif actual_sub_t == "โจทย์ปัญหาอัตราส่วน":
                ...
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                <b>ขั้นที่ 1: แปลความหมายของอัตราส่วน</b><br>
                👉 จากอัตราส่วน ชาย : หญิง = {a} : {b} (หมายความว่าถ้าจัดเป็นกลุ่มๆ กลุ่มชายจะมี {a} ส่วน และกลุ่มหญิงจะมี {b} ส่วน)<br>
                <b>ขั้นที่ 2: เทียบหาปริมาณใน 1 ส่วน</b><br>
                👉 โจทย์บอกว่าของจริงมีนักเรียนชายอยู่ {a*m} คน<br>
                👉 เราต้องหาว่า 1 ส่วนมีค่าเท่ากับกี่คน โดยนำจำนวนคนจริงไปหารกับจำนวนส่วน ➔ {a*m} คน ÷ {a} ส่วน = <b>{m} คนต่อ 1 ส่วน</b><br>
                <b>ขั้นที่ 3: นำไปคูณเพื่อหาคำตอบ</b><br>
                👉 เมื่อรู้ว่า 1 ส่วนคือ {m} คน ให้นำไปคูณกับจำนวนส่วนของนักเรียนหญิง ({b} ส่วน)<br>
                👉 นักเรียนหญิง = {b} × {m} = <b>{b*m} คน</b><br>
                <b>ตอบ: มีนักเรียนหญิงทั้งหมด {b*m} คน</b></span>"""

            elif actual_sub_t == "โจทย์ปัญหาร้อยละ":
                product_type = random.choice([
                    ("สินค้า", ITEMS),
                    ("เครื่องใช้ไฟฟ้า", ["โทรทัศน์", "ตู้เย็น", "เครื่องซักผ้า", "พัดลม", "เตารีด"]),
                    ("เสื้อผ้าแฟชั่น", ["รองเท้าผ้าใบ", "กระเป๋าเป้", "เสื้อแจ็คเก็ต"])
                ])
                product_name = random.choice(product_type[1])
                base_price = random.choice([890, 1250, 1500, 2400, 3500, 5990, 8500, 12000, 15900, 24500])
                percent = random.choice([5, 10, 12, 15, 20, 25, 30, 40, 50])
                scenario = random.choice(["ลดราคา", "กำไร", "ขาดทุน"])
                amount_change = int(base_price * (percent / 100))
                
                frac_percent = f_html(percent, 100, color="#2c3e50")
                
                if scenario == "ลดราคา":
                    q = f"ห้างสรรพสินค้าแห่งหนึ่งติดป้ายราคา<b>{product_name}</b>ไว้ที่ <b>{base_price:,} บาท</b> แต่เนื่องในเทศกาลปีใหม่ทางห้างประกาศ<b>ลดราคาให้ {percent}%</b> <br>อยากทราบว่าลูกค้าจะได้รับส่วนลดกี่บาท?"
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    <b>ขั้นที่ 1: แปลงความหมายของเปอร์เซ็นต์ให้เป็นเศษส่วน</b><br>
                    👉 คำว่า "ลดราคา {percent}%" หมายถึง ถ้าราคาสินค้า 100 บาท จะลดราคาให้ {percent} บาท<br>
                    👉 เขียนให้อยู่ในรูปเศษส่วนจะได้ <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> ของราคาป้าย<br>
                    <b>ขั้นที่ 2: ตั้งสมการเพื่อหาจำนวนเงินที่ลดราคา</b><br>
                    👉 นำเศษส่วนที่ได้ ไปคูณกับราคาป้ายจริง คือ {base_price:,} บาท<br>
                    👉 ประโยคสัญลักษณ์: <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> × {base_price:,} = 🔲<br>
                    <b>ขั้นที่ 3: คำนวณหาผลลัพธ์</b><br>
                    👉 นำตัวเศษคูณกับราคาป้าย: {percent} × {base_price:,} = {percent * base_price:,}<br>
                    👉 นำผลคูณที่ได้มาหารด้วยตัวส่วน (100): {percent * base_price:,} ÷ 100 = <b>{amount_change:,} บาท</b><br>
                    <b>ตอบ: ลูกค้าจะได้รับส่วนลด {amount_change:,} บาท</b></span>"""
                elif scenario == "กำไร":
                    q = f"พ่อค้าซื้อ<b>{product_name}</b>มาในราคาทุน <b>{base_price:,} บาท</b> เมื่อนำไปขายต่อ เขาได้<b>กำไร {percent}%</b> <br>อยากทราบว่าพ่อค้าได้กำไรจากการขายครั้งนี้กี่บาท?"
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    <b>ขั้นที่ 1: แปลงความหมายของเปอร์เซ็นต์ให้เป็นเศษส่วน</b><br>
                    👉 คำว่า "กำไร {percent}%" หมายถึง ถ้าราคาทุน 100 บาท จะขายได้กำไร {percent} บาท<br>
                    👉 เขียนให้อยู่ในรูปเศษส่วนจะได้ <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> ของราคาทุน<br>
                    <b>ขั้นที่ 2: ตั้งสมการเพื่อหาจำนวนเงินกำไร</b><br>
                    👉 นำเศษส่วนที่ได้ ไปคูณกับราคาทุนจริง คือ {base_price:,} บาท<br>
                    👉 ประโยคสัญลักษณ์: <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> × {base_price:,} = 🔲<br>
                    <b>ขั้นที่ 3: คำนวณหาผลลัพธ์</b><br>
                    👉 นำตัวเศษคูณกับราคาทุน: {percent} × {base_price:,} = {percent * base_price:,}<br>
                    👉 นำผลคูณที่ได้มาหารด้วยตัวส่วน (100): {percent * base_price:,} ÷ 100 = <b>{amount_change:,} บาท</b><br>
                    <b>ตอบ: พ่อค้าได้กำไรทั้งหมด {amount_change:,} บาท</b></span>"""
                else: 
                    q = f"แม่ค้าซื้อ<b>{product_name}</b>มาในราคาทุน <b>{base_price:,} บาท</b> แต่เนื่องจากสินค้ามีตำหนิ จึงต้องยอมขาย<b>ขาดทุน {percent}%</b> <br>อยากทราบว่าแม่ค้าขายขาดทุนไปกี่บาท?"
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    <b>ขั้นที่ 1: แปลงความหมายของเปอร์เซ็นต์ให้เป็นเศษส่วน</b><br>
                    👉 คำว่า "ขาดทุน {percent}%" หมายถึง ถ้าราคาทุน 100 บาท จะเสียเงินทุนไป {percent} บาท<br>
                    👉 เขียนให้อยู่ในรูปเศษส่วนจะได้ <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> ของราคาทุน<br>
                    <b>ขั้นที่ 2: ตั้งสมการเพื่อหาจำนวนเงินที่ขาดทุน</b><br>
                    👉 นำเศษส่วนที่ได้ ไปคูณกับราคาทุนจริง คือ {base_price:,} บาท<br>
                    👉 ประโยคสัญลักษณ์: <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> × {base_price:,} = 🔲<br>
                    <b>ขั้นที่ 3: คำนวณหาผลลัพธ์</b><br>
                    👉 นำตัวเศษคูณกับราคาทุน: {percent} × {base_price:,} = {percent * base_price:,}<br>
                    👉 นำผลคูณที่ได้มาหารด้วยตัวส่วน (100): {percent * base_price:,} ÷ 100 = <b>{amount_change:,} บาท</b><br>
                    <b>ตอบ: แม่ค้าขายขาดทุนไป {amount_change:,} บาท</b></span>"""
...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังออกแบบรูปภาพและสร้างเฉลยแบบ Step-by-Step..."):
        ...
        # ⚠️ แก้ไขจุดที่ทำให้แอปพัง: ส่งตัวแปร 4 ตัวให้ตรงกับที่ฟังก์ชันต้องการรับเป๊ะๆ
        ...
        filename_base = f"KingMath_Std_{selected_grade}_{selected_sub}"
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ สร้างไฟล์สำเร็จ! ระบบเสถียร 100% ล้างบั๊กตัวแปรและวาดรูปนาฬิกา/ตาชั่ง/แผนภูมิตามที่คุณต้องการเรียบร้อยแล้วครับ")
    c1, c2 = st.columns(2)
    with c1:

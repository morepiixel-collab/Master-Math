...
def generate_unit_math_html(u_maj, u_min, v1_maj, v1_min, v2_maj, v2_min, op, multiplier):
    """ตัวช่วยสำหรับวาดตารางตั้งบวกลบของหน่วยการวัดข้ามหน่วย (เช่น เมตร-เซนติเมตร)"""
    if op == "+":
        ...
        while attempts < 300:
            ...
            if actual_sub_t == "ปริมาตรและความจุ (มิลลิลิตร ลิตร)":
                ...
                if q_cat == "compare":
                    ...
                    👉 <b>สมการล่าสุด:</b> ({val_major} × {multiplier:,}) + {val_minor} = {val_major * multiplier:,} + {val_minor} = <b>{total_minor_1:,} {u_minor}</b><br>
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดอัปเดตเรียบร้อยครับ! รักษาสถานะโค้ดเดิม 100% พร้อมเพิ่มเรื่องปริมาตรและกราฟิกถังน้ำเรียบร้อยแล้วครับ")
    c1, c2 = st.columns(2)
    with c1:

...
def cm_to_m_cm_mm(cm_float):
    """ตัวช่วยแปลงหน่วย ซม. เป็น เมตร เซนติเมตร มิลลิเมตร"""
...
def draw_ruler_svg(start_cm, end_cm):
    """สำหรับไม้บรรทัดแบบสั้น (เห็นทั้งชิ้น)"""
    scale = 40  
...
def draw_long_ruler_svg(length_cm, color="#f1c40f", name=""):
    """สำหรับสายวัดแบบยาว (แสดงแค่ช่วงปลายทะลุ 100 ซม.)"""
    scale = 40
    ...
    # วาดสิ่งของทะลุมาจากขอบซ้าย (x=0)
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ เพิ่มหน่วยเมตรสำหรับ ป.3 และโหมดเปรียบเทียบสิ่งของ (Challenge) เรียบร้อยแล้วครับ!")
    c1, c2 = st.columns(2)
    with c1:

...
def generate_questions_logic(grade, main_t, sub_t, num_q):
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # =========================================================
            # ฐานข้อมูลโจทย์ทั้งหมด (ไม่มีการข้ามบรรทัด ไม่มีการตัดทอน)
            # =========================================================
            if actual_sub_t == "การคูณ (แบบตั้งหลัก)":
                ...
                <b>ขั้นที่ 2:</b> แปลงหน่วยขีดเป็นหน่วยกรัม (จำไว้ว่า 1 ขีด = 100 กรัมเสมอ)<br>
                ...
                product_type = random.choice([
                    ("สินค้า", items_shopping + items_fashion),
                    ("เครื่องใช้ไฟฟ้า", items_shopping),
                    ("เสื้อผ้าแฟชั่น", items_fashion)
                ])
                ...
                if op == "*":
                    ...
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    เป้าหมายคือต้องทำให้ x อยู่คนเดียวโดดๆ ทางฝั่งซ้าย<br>
                    <b>ขั้นที่ 1:</b> จากโจทย์ <span style='display:inline-flex; align-items:center;'>{f_html('x', str(a), '#2c3e50')}</span> หมายความว่า x กำลังถูก "หาร" ด้วยเลข <b>{a}</b> เราจึงต้องกำจัดเลข {a} ที่เป็นตัวส่วนนี้ออกไป<br>
                    <b>ขั้นที่ 2:</b> โดยใช้สมบัติการเท่ากัน: เราจะนำ <b>{a} มาคูณเข้าทั้งสองข้างของสมการ</b><br>
                    <b>ขั้นที่ 3:</b> เขียนบรรทัดใหม่ได้ว่า: <span style='display:inline-flex; align-items:center;'>{f_html('x', str(a), '#2c3e50')}</span> <b style='color:green;'>× {a}</b> = {x} <b style='color:green;'>× {a}</b><br>
                ...
                # ป้องกัน Error หากเกิดเหตุการณ์ไม่คาดคิด
                a, b = random.randint(10, 50), random.randint(10, 50)
                q = f"จงหาผลลัพธ์ <span style='display:inline-flex; align-items:center; font-weight: bold; color: #2c3e50; margin-left: 5px;'>{a} + {b} = {box_html}</span>"
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดสมบูรณ์เต็มรูปแบบ! จัดเต็มเฉลยละเอียดยิบแบบ Step-by-step และแปลงเศษส่วนเป็นแนวตั้ง 100% เรียบร้อยแล้วครับ")
    c1, c2 = st.columns(2)
    with c1:

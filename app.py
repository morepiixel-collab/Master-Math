...
<div class="main-header">
    ...
    <p>ระบบสร้างสื่อการสอนคณิตศาสตร์ (ป.1 - ป.6) พร้อมระบบโจทย์แข่งขัน TMC, Branding และ Spacing แบบละเอียด</p>
</div>
""", unsafe_allow_html=True)
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            if sub_t == "แบบทดสอบรวมปลายภาค":
                ...
                actual_sub_t = random.choice([
                    "ปริศนาตัวเลขซ่อนแอบ", "การนับหน้าหนังสือ", "การปักเสาและปลูกต้นไม้", "หอยทากปีนบ่อ",
                    "ตรรกะตาชั่งสมดุล", "อายุข้ามเวลาขั้นสูง", "การตัดเชือกพับทบ", "แถวคอยแบบซ้อนทับ"
                ])

            prefix = get_prefix(grade)

            # ==============================
            # 🌟 โหมดข้อสอบแข่งขัน (TMC) - 8 หัวข้อสุดโหด!
            # ==============================
            if actual_sub_t == "ปริศนาตัวเลขซ่อนแอบ":
                ...
                q = f"หนังสือเล่มหนึ่งมีความหนาทั้งหมด <b>{pages}</b> หน้า หากโรงพิมพ์ต้องการพิมพ์ตัวเลขเพื่อบอกเลขหน้าทั้งหมดตั้งแต่หน้า 1 ไปจนถึงหน้า {pages} โรงพิมพ์จะต้องใช้ตัวเลขโดดทั้งหมดกี่ตัว?"
                if pages > 99:
                ...
                length = (trees - 1) * d
                q = f"เทศบาลต้องการปลูกต้นไม้ริมถนนฝั่งหนึ่ง โดยให้ต้นไม้แต่ละต้นอยู่ห่างกันต้นละ <b>{d}</b> เมตร และต้องปลูกที่จุดเริ่มต้นและจุดสิ้นสุดของถนนพอดี หากเมื่อปลูกเสร็จแล้วนับจำนวนต้นไม้ได้ทั้งหมด <b>{trees}</b> ต้น จงหาว่าถนนเส้นนี้มีความยาวกี่เมตร?"
            ...
            elif actual_sub_t == "หอยทากปีนบ่อ":
                u = random.randint(3, 7)
                d = random.randint(1, u - 1)
                net = u - d
                h = random.randint(15, 30)
                days = math.ceil((h - u) / net) + 1
                q = f"หอยทากตัวหนึ่งตกลงไปในบ่อที่ลึก <b>{h}</b> เมตร ในช่วงเวลากลางวันหอยทากมีความพยายามปีนขึ้นมาได้ <b>{u}</b> เมตร แต่เมื่อถึงเวลากลางคืนหอยทากต้องนอนหลับ ทำให้ลื่นไถลตกลงไป <b>{d}</b> เมตร หอยทากจะต้องใช้เวลาอย่างน้อยที่สุดกี่วันจึงจะปีนพ้นปากบ่อ?"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>หอยทากปีนได้วันละ {u} เมตร และลื่นลง {d} เมตร สรุป 1 วัน (24 ชม.) ปีนได้สุทธิ {u} - {d} = <b>{net} เมตร</b><br><i>*จุดหลอก:</i> ในวันสุดท้าย เมื่อหอยทากปีนพ้นปากบ่อแล้ว จะไม่ลื่นตกลงมาอีก!<br>หาระยะทางก่อนถึงวันสุดท้าย = ความลึกบ่อ {h} - ปีนวันสุดท้าย {u} = <b>{h - u} เมตร</b><br>เวลาที่ใช้ปีนช่วงแรก = {h - u} ÷ {net} = {math.ceil((h - u) / net)} วัน<br>บวกกับวันสุดท้ายที่ปีนพ้นบ่ออีก 1 วัน = {math.ceil((h - u) / net)} + 1 = </span><b>{days} วัน</b>"

            elif actual_sub_t == "ตรรกะตาชั่งสมดุล":
                items = [("แอปเปิล", "กล้วย", "ส้ม"), ("มังคุด", "ชมพู่", "เงาะ"), ("แตงโม", "สับปะรด", "มะละกอ")]
                i1, i2, i3 = random.choice(items)
                mul1 = random.randint(2, 5)
                mul2 = random.randint(2, 5)
                q = f"จากการชั่งน้ำหนักผลไม้เพื่อหาความสมดุล พบข้อมูลดังนี้:<br>- <b>{i1} 1 ผล</b> มีน้ำหนักเท่ากับ <b>{i2} {mul1} ผล</b><br>- <b>{i2} 1 ผล</b> มีน้ำหนักเท่ากับ <b>{i3} {mul2} ผล</b><br><br>อยากทราบว่า <b>{i1} จำนวน 2 ผล</b> จะมีน้ำหนักเท่ากับ <b>{i3}</b> รวมกันทั้งหมดกี่ผล?"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>เราต้องแปลงน้ำหนักให้เป็นหน่วยของ {i3} ทั้งหมด:<br>จากบรรทัดที่สอง: {i2} 1 ผล = {i3} {mul2} ผล<br>นำไปแทนค่าในบรรทัดแรก: {i1} 1 ผล = {i2} {mul1} ผล = {mul1} × {mul2} = <b>{i3} {mul1 * mul2} ผล</b><br>โจทย์ถามหา {i1} <b>2 ผล</b> <br>ดังนั้น ต้องนำ {mul1 * mul2} × 2 = </span><b>{mul1 * mul2 * 2} ผล</b>"
                ...
                q = f"ปัจจุบัน ครอบครัวหนึ่งมีพี่น้อง 3 คน คือ เอ บี และซี ซึ่งเมื่อนำอายุของทั้งสามคนมารวมกันจะได้ <b>{current_sum}</b> ปีพอดี <br>จงหาว่าเมื่อ <b>{past_years}</b> ปีที่แล้ว เด็กทั้งสามคนนี้มีอายุรวมกันเป็นกี่ปี? (กำหนดให้เมื่อ {past_years} ปีที่แล้ว น้องคนเล็กสุดเกิดและมีอายุแล้ว)"
                ...
                q = f"นักเรียนคนหนึ่งนำเชือกเส้นยาวเส้นหนึ่งมาพับทบครึ่งกันจำนวน <b>{folds}</b> ครั้ง (พับครึ่งแล้วพับครึ่งซ้ำไปเรื่อยๆ) จากนั้นนำกรรไกรมาตัดแบ่งเชือกที่พับไว้ให้ขาดออกจากกันจำนวน <b>{cuts}</b> รอยตัด <br>เมื่อคลี่เชือกทั้งหมดออกมา นักเรียนจะได้เชือกทั้งหมดกี่เส้น?"
                ...
                q = f"นักเรียนกลุ่มหนึ่งยืนเข้าแถวตอนเรียงหนึ่งเพื่อรอซื้ออาหาร โดยมีนักเรียนทั้งหมดในแถว <b>{total_people}</b> คน<br>ถ้า <b>มังกร</b> ยืนอยู่ในลำดับที่ <b>{front_pos}</b> นับจากหัวแถว <br>และ <b>ฉลาม</b> ยืนอยู่ในลำดับที่ <b>{back_pos}</b> นับจากท้ายแถว <br>อยากทราบว่ามีนักเรียนคนอื่นๆ ที่ยืนอยู่ระหว่าง มังกร กับ ฉลาม เป็นจำนวนกี่คน? (กำหนดให้มังกรยืนอยู่หน้าฉลาม)"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>เราสามารถหาจำนวนคนที่อยู่ตรงกลางได้โดยการหักคนที่อยู่ด้านหน้าและด้านหลังออกไป<br>1) มังกรอยู่ลำดับที่ {front_pos} จากหัวแถว แสดงว่าตั้งแต่หัวแถวจนถึงตัวมังกร มีคนรวม <b>{front_pos} คน</b><br>2) ฉลามอยู่ลำดับที่ {back_pos} จากท้ายแถว แสดงว่าตั้งแต่ท้ายแถวจนถึงตัวฉลาม มีคนรวม <b>{back_pos} คน</b><br>3) นำคนทั้งแถว ลบด้วยกลุ่มด้านหน้าและกลุ่มด้านหลัง<br>จำนวนคนตรงกลาง = {total_people} - ({front_pos} + {back_pos})<br>= {total_people} - {front_pos + back_pos} = </span><b>{between} คน</b>"
...
if worksheet_mode == "📚 หลักสูตรปกติ (ป.1 - ป.6)":
    ...
    if "🌟 โจทย์แข่งขัน (แนว TMC)" in main_topics_list:
        main_topics_list.remove("🌟 โจทย์แข่งขัน (แนว TMC)") # ซ่อนเมนูแข่งขันออกจากโหมดปกติ
    ...
    selected_sub = st.sidebar.selectbox("📝 เลือกแนวข้อสอบ:", [
        "ปริศนาตัวเลขซ่อนแอบ", "การนับหน้าหนังสือ", "การปักเสาและปลูกต้นไม้", 
        "หอยทากปีนบ่อ", "ตรรกะตาชั่งสมดุล", "อายุข้ามเวลาขั้นสูง", 
        "การตัดเชือกพับทบ", "แถวคอยแบบซ้อนทับ", "🌟 สุ่มรวมทุกแนว"
    ])
    if selected_sub == "🌟 สุ่มรวมทุกแนว":
        st.sidebar.info("💡 โหมดนี้จะสุ่มข้อสอบแข่งขันทั้ง 8 แนวมาคละกัน ให้เด็กๆ ได้ฝึกกระบวนการคิดที่หลากหลายแบบจัดเต็ม!")
...
# แมปค่าระยะห่างกับความสูงของพื้นที่ทดเลขให้สัมพันธ์กัน
...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังประมวลผลลอจิกคณิตศาสตร์และวาดกราฟิก..."):
        ...
        # ส่งค่า ws_height และ q_margin เข้าไปตอนสร้างหน้ากระดาษ

...
# 🏠 เพิ่มคลังคำศัพท์สำหรับหมวดหมู่ความยาวและการวัด (เฟอร์นิเจอร์และห้อง)
...
curriculum_db = {
    ...
    "ป.3": {
        ...
        # 📌 เพิ่มหัวข้อย่อย "โจทย์ปัญหาความยาว (คูณและหาร)" สำหรับ ป.3
...
def generate_questions_logic(grade, main_t, sub_t, num_q, is_challenge=False):
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # 📌 เริ่มหัวข้อใหม่: โจทย์ปัญหาความยาว (คูณและหาร)
            if actual_sub_t == "โจทย์ปัญหาความยาว (คูณและหาร)":
                ...
                if q_type == "fit_objects":
                    ...
                    if is_challenge:
                        # ห้องกว้างขึ้น เฟอร์นิเจอร์ตัวเลขไม่ลงตัวง่ายๆ
                    ...
                    if f_m == 0 and f_cm < 40: f_cm = 50 # ป้องกันเฟอร์นิเจอร์เล็กเกินไป
                    if f_m == 1 and f_cm == 100: f_m, f_cm = 2, 0
                    
                    room_total_cm = r_m * 100 + r_cm
                    furn_total_cm = f_m * 100 + f_cm
                    
                    # คำนวณหาจำนวนตัวและพื้นที่เหลือ
                ...
                if q_type == "convert_to_km":
                    ...
                    # 🔥 สุ่มถามว่า ไกลกว่า หรือ สั้นกว่า(ใกล้กว่า)
                ...
                if grade == "ป.3":
                    ...
                    if is_challenge:
                        ...
                        # 🔥 สุ่มคำถามให้หลากหลายว่า 'มากกว่ากัน' หรือ 'น้อยกว่ากัน'
                ...
                if q_type == "single":
                    ...
                    # 🔥 สุ่มถามมากกว่า/น้อยกว่าเพื่อความหลากหลาย
                    is_more = random.choice([True, False])
                    if is_more:
                        compare_word = "มากกว่า"
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดอัปเดตเรียบร้อยครับ! มีเรื่องการวัดที่ประยุกต์เข้ากับพื้นที่และการจัดวางพร้อมตัวเลขที่หลากหลาย")
    c1, c2 = st.columns(2)
    with c1:

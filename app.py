...
def get_fraction_solution_steps(num, den):
    ...
    if sim_den == 1:
        ...
        if g > 1: 
            extra_steps = f"ทอนเป็นเศษส่วนอย่างต่ำ (นำ {g} มาหารทั้งตัวเศษและตัวส่วน) จะได้ผลลัพธ์เป็นจำนวนเต็ม"
    elif sim_num > sim_den:
        w = sim_num // sim_den
        r = sim_num % sim_den
        final_html = generate_mixed_number_html(w, r, sim_den)
        if g > 1: 
            extra_steps = f"ทอนเป็นเศษส่วนอย่างต่ำ (นำ {g} มาหารทั้งตัวเศษและตัวส่วน) และนำไปตั้งหารเพื่อแปลงเศษเกินให้อยู่ในรูปจำนวนคละ"
        else: 
            extra_steps = f"นำไปตั้งหารเพื่อแปลงเศษเกินให้อยู่ในรูปจำนวนคละ"
    else:
        ...
        if g > 1: 
            extra_steps = f"ทอนเป็นเศษส่วนอย่างต่ำโดยนำ {g} มาหารทั้งตัวเศษและตัวส่วน"
...
def generate_questions_logic(grade, main_t, sub_t, num_q):
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # =========================================================
            # โหมดหลักสูตรปกติ (แก้ไขเงื่อนไขดักจับให้ตรงกับ Dictionary 100%)
            # =========================================================
            if actual_sub_t == "การคูณ (แบบตั้งหลัก)":
            ...
            elif "การเรียงลำดับจำนวน" in actual_sub_t or "การเปรียบเทียบและเรียงลำดับ" in actual_sub_t:
                ...
                # หากเป็น ป.5 ขึ้นไปและหัวข้อคือ บวก/ลบ เศษส่วน ต้องทำส่วนให้เท่ากันก่อน
                ...
                # Fallback ป้องกัน Error หากเกิดเหตุการณ์ไม่คาดคิด
...
if worksheet_mode == "📚 หลักสูตรปกติ (ป.1 - ป.6)":
    ...
    if "ป.1" in selected_grade:
        sub_options = tmc_p1_p2
    elif "ป.3" in selected_grade:
        sub_options = tmc_p3_p4
    else:
        sub_options = tmc_p5_p6

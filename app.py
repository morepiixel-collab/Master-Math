...
# ==========================================
# ตั้งค่าหน้าเพจ Web App & Custom CSS (Professional UI)
# ==========================================
...
<div class="main-header">
    ...
    <p>ระบบสร้างสื่อการสอนอัตโนมัติ พร้อมระบบ Branding, Spacing, โหมดทดสอบรวม และ PDF Ready สำหรับ TPT</p>
</div>
""", unsafe_allow_html=True)
...
# 💡 อัปเกรดลอจิก: รองรับ "โหมดทดสอบรวมปลายภาค" (สุ่มจากทุกเรื่อง)
...
def create_page(grade, sub_t, questions, is_key=False, q_margin="60px", brand_name=""):
    ...
    <style>
        ...
        .q-box {{ margin-bottom: {q_margin}; padding: 15px; page-break-inside: avoid; border-bottom: 1px solid #eee; }}
    ...
    # เพิ่ม Footer ลิขสิทธิ์ที่ท้ายกระดาษ
    if brand_name:
...
def generate_cover_html(grade, main_t, sub_t, num_q, theme_colors, brand_name):
    ...
    <style>
        .cover-inner {{ width: 100%; height: 100%; padding: 40px; box-sizing: border-box; text-align: center; position: relative; border: 15px solid {border_color}; background: white; }}
        .title-box {{ margin-top: 80px; }}
        .title {{ font-size: 65px; color: #2c3e50; font-weight: bold; margin: 0; line-height: 1.2; }}
        .grade-badge {{ font-size: 45px; background-color: {badge_color}; color: white; padding: 15px 50px; border-radius: 50px; display: inline-block; font-weight: bold; margin-top: 30px; }}
...
if selected_main == "🌟 โหมดพิเศษ (สุ่มทุกเรื่อง)":
    ...
    st.sidebar.info("💡 โหมดนี้จะสุ่มดึงโจทย์จากทุกเรื่องในชั้นเรียนนี้มายำรวมกัน เหมาะสำหรับทำข้อสอบปลายภาค!")
else:
...
spacing_level = st.sidebar.select_slider("↕️ ระยะห่างระหว่างข้อ (พื้นที่ทด):", options=["แคบ", "ปานกลาง", "กว้าง"], value="ปานกลาง")

spacing_map = { "แคบ": "30px", "ปานกลาง": "60px", "กว้าง": "120px" }
q_margin = spacing_map[spacing_level]
...
theme_colors = theme_map[color_theme]
...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังประมวลผลลอจิกคณิตศาสตร์และวาดกราฟิก..."):
        ...
        html_cover = generate_cover_html(selected_grade, selected_main, selected_sub, num_input, theme_colors, brand_name) if include_cover else ""
        
        # เก็บไฟล์แยกลง Session State เพื่อให้ดาวน์โหลดแยกส่วนได้
        st.session_state['worksheet_html'] = html_w
        st.session_state['answerkey_html'] = html_k
        
        # --- รวมร่าง E-Book และเพิ่ม CSS ควบคุมหน้าจอ (A4 Preview) ---
        ebook_body = ""
        if include_cover:
        ...
        <style>
            /* 🖨️ 1. ตั้งค่ากระดาษ A4 เมื่อกด Print */
            @page {{ size: A4; margin: 15mm; }}
            
            /* 🖥️ 2. ตั้งค่าการแสดงผลบนหน้าจอคอมพิวเตอร์ (จำลองการดูไฟล์ PDF) */
            @media screen {{
            ...
            /* 🖨️ 3. ตั้งค่าเมื่อกด Print (Ctrl+P) ให้พอดี A4 ไร้ขอบ */
            @media print {{
...
# ==========================================
# ส่วนจัดการผลลัพธ์ (Download & Preview)
# ==========================================
if 'ebook_html' in st.session_state:
    st.success(f"✅ สร้างไฟล์สำเร็จ! ชุดใบงาน **{st.session_state['filename_base'].split('_')[1]}** พร้อมให้ดาวน์โหลดแล้ว")
    
    st.markdown("### 📥 เลือกรูปแบบดาวน์โหลด (HTML สำหรับ Print PDF)")
    
    col_dl1, col_dl2 = st.columns(2)
    
    with col_dl1:
        st.info("📄 **ไฟล์แยกส่วน** (สำหรับนำไปแจกนักเรียน หรือจัดหน้าเอง)")
        st.download_button(
            label="📄 โหลดเฉพาะโจทย์ (Worksheet)",
        ...
        st.download_button(
            label="🔑 โหลดเฉพาะเฉลย (Answer Key)",
    ...
    with col_dl2:
        st.info("📚 **ไฟล์รวมเล่ม / แพ็กเกจ** (พร้อมทำเล่มขาย / Print รวดเดียว)")
        st.download_button(
            label="📚 โหลด Full E-Book (หน้าปก+โจทย์+เฉลย)",
        ...
        st.download_button(
            label="🗂️ โหลดแพ็กเกจทั้งหมด (.zip)",
    ...
    st.caption("จำลองการแสดงผลบนกระดาษ A4 (สามารถเลื่อนดูหน้าปก ข้อสอบ และเฉลยได้เลย)")
    
    components.html(st.session_state['ebook_html'], height=800, scrolling=True)

else:
    st.info("👈 กรุณาตั้งค่าใบงานที่แถบเมนูด้านซ้าย แล้วกดปุ่ม สีเขียว 'สั่งสร้างใบงานเดี๋ยวนี้'")

...
# ==========================================
# 2. ฐานข้อมูลหัวข้อหลักสูตร (Master Database) ครบถ้วน
# ==========================================
curriculum_db = {
    "ป.1": {
...
def generate_questions_logic(grade, main_t, sub_t, num_q):
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # =========================================================
            # โหมดหลักสูตรปกติ (ตรวจสอบการดักจับชื่อหัวข้อให้ตรงกับ Database 100%)
            # =========================================================
            if actual_sub_t == "การคูณ (แบบตั้งหลัก)":
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-Step):</b><br>1) การหารคือการหาว่า 'ตัวหาร ({a}) ต้องคูณกับเลขอะไรจึงจะได้ผลลัพธ์เท่ากับตัวตั้ง ({dividend})'<br>2) ให้นักเรียนลองท่องสูตรคูณแม่ <b>{a}</b> ดูครับ:<br>&nbsp;&nbsp;&nbsp;{a} × 1 = {a}<br>&nbsp;&nbsp;&nbsp;...<br>&nbsp;&nbsp;&nbsp;<b>{a} × {b} = {dividend}</b> (เจอคำตอบแล้ว!)<br>ดังนั้น {dividend} ÷ {a} มีค่าเท่ากับ <b>{b}</b><br><b>ตอบ: {b}</b></span>"
                ...
                q = f"เวลา <b>{h_24:02d}:{m:02d} น.</b> อ่านว่าอย่างไร?"
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                1) ตัวเลขหน้าเครื่องหมาย : บ่งบอกถึงหน่วย 'นาฬิกา' คือ <b>{h_24}</b><br>
                2) ตัวเลขหลังเครื่องหมาย : บ่งบอกถึงหน่วย 'นาที' คือ <b>{m}</b><br>
                3) นำมาอ่านรวมกัน จะได้ <b>{h_24} นาฬิกา {m} นาที</b><br>
                <b>ตอบ: {h_24} นาฬิกา {m} นาที</b></span>"""

            elif actual_sub_t == "การอ่านน้ำหนักจากเครื่องชั่งสปริง":
                kg = random.randint(1, 5)
                kheed = random.randint(1, 9)
                g = kheed * 100
                q = f"เมื่อวางของบนเครื่องชั่งสปริง เข็มชี้เลยตัวเลข {kg} ไปอีก <b>{kheed} ขีด</b> สิ่งของนี้หนักเท่าไร? (ตอบเป็นกิโลกรัมและกรัม)"
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                1) เข็มชี้เลยเลข {kg} แปลว่ามีน้ำหนักหลักคือ <b>{kg} กิโลกรัม</b><br>
                2) แปลงหน่วยขีดเป็นหน่วยกรัม (จำไว้ว่า 1 ขีด = 100 กรัมเสมอ)<br>
                👉 นำ {kheed} ขีด × 100 = <b>{g} กรัม</b><br>
                3) นำน้ำหนักมารวมกัน ➔ <b>{kg} กิโลกรัม {g} กรัม</b><br>
                ...
                q = f"ในแผนภูมิรูปภาพ กำหนดให้รูป <b>{item} 1 รูป แทนจำนวน {pic_val} ผล</b><br>ถ้าในตารางวาดรูป <b>{item} ไว้ทั้งหมด {count} รูป</b> จะมี{item}จริงๆ ทั้งหมดกี่ผล?"
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                1) ตรวจสอบข้อกำหนดของแผนภูมิ: รูป 1 รูป ไม่ได้แปลว่า 1 ผล แต่โจทย์กำหนดให้ 1 รูป = <b>{pic_val} ผล</b><br>
                2) นับจำนวนรูปในตาราง: มีรูปวาดทั้งหมด <b>{count} รูป</b><br>
                3) คำนวณหาจำนวนที่แท้จริง: นำจำนวนรูปภาพ × ข้อกำหนด ➔ {count} × {pic_val} = <b>{count * pic_val} ผล</b><br>
                ...
                if is_peri:
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    1) นึกถึงสูตรการหาความยาวรอบรูปสี่เหลี่ยมจัตุรัส<br>
                    ...
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    1) นึกถึงสูตรการหาพื้นที่สี่เหลี่ยมจัตุรัส<br>
                ...
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                1) หาตัวส่วน (จำนวนช่องทั้งหมด)<br>
                ...
                if grade == "ป.5" and actual_sub_t in ["การบวกเศษส่วน", "การลบเศษส่วน"]:
                    ...
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    1) ตรวจสอบและหา ค.ร.น. ของตัวส่วน<br>
                    ...
                    2) แปลงเศษส่วนให้มีตัวส่วนเท่ากับ {lcm_d}<br>
                ...
                if op == '×':
                    ...
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    1) เข้าใจหลักการคูณเศษส่วน<br>
                    👉 การคูณเศษส่วนไม่ต้องทำตัวส่วนให้เท่ากัน สามารถจับคู่คูณได้เลย คือ "เศษคูณเศษ" และ "ส่วนคูณส่วน"<br>
                    2) นำตัวเศษ (เลขด้านบน) มาคูณกัน<br>
                    👉 {n1} × {n2} = <b>{n1*n2}</b><br>
                    3) นำตัวส่วน (เลขด้านล่าง) มาคูณกัน<br>
                    👉 {d1} × {d2} = <b>{d1*d2}</b><br>
                    4) ประกอบร่างเป็นเศษส่วนผลลัพธ์<br>
                ...
                if is_front:
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    1) สังเกตความสัมพันธ์ของตัวเลขที่มีคู่ครบ<br>
                ...
                frac_percent = f_html(percent, 100, color="#2c3e50", is_bold=False)
                ...
                if scenario == "ลดราคา":
                    ...
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    1) แปลงความหมายของเปอร์เซ็นต์ให้เป็นเศษส่วน<br>
                    ...
                    2) ตั้งสมการเพื่อหาจำนวนเงินที่ลดราคา<br>
                    ...
                    2) ตั้งสมการเพื่อหาจำนวนเงินกำไร<br>
                    ...
                    👉 เขียนให้อยู่ในรูปเศษส่วนจะได้ <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> ของราคาทุน<br>
                    2) ตั้งสมการเพื่อหาจำนวนเงินที่ขาดทุน<br>
                    👉 นำเศษส่วนที่ได้ ไปคูณกับราคาทุนจริง คือ {base_price:,} บาท<br>
                    👉 ประโยคสัญลักษณ์: <span style='display:inline-flex; align-items:center;'>{frac_percent}</span> × {base_price:,} = 🔲<br>
                    3) คำนวณหาผลลัพธ์<br>
                    👉 นำตัวเศษคูณกับราคาทุน: {percent} × {base_price:,} = {percent * base_price:,}<br>
                ...
                sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                1) กฎเหล็กของการบวกลบทศนิยมคือ <b>ต้องตั้งจุดทศนิยมให้ตรงกันเป๊ะๆ</b> เสมอ!<br>
                2) ทำการตั้งหลักตัวเลขอื่นๆ ให้ตรงกันตามจุดทศนิยม<br>
                3) ทำการ{word_op}เลขจากขวาไปซ้ายตามปกติเหมือนการ{word_op}เลขจำนวนเต็มครับ</span><br>{sol_table}"""
                ...
                1) นำ {int(round(a*10))} มาตั้งคูณด้วย {b} ได้ผลลัพธ์ดังนี้:<br>{sol_table}<br>
                2) จากนั้นมาพิจารณาดูว่าตัวตั้งและตัวคูณรวมกันมีทศนิยมกี่ตำแหน่ง<br>
                👉 ในข้อนี้ {a:.1f} มีทศนิยม 1 ตำแหน่ง และ {b} ไม่มีทศนิยม (0 ตำแหน่ง) รวมเป็น 1 ตำแหน่ง<br>
                3) ให้นำผลลัพธ์ที่ได้ มาใส่จุดทศนิยม 1 ตำแหน่ง (นับจากหลังมาหน้า)<br>
                ...
                1) ตัวเลขหน้าจุดให้อ่านแบบจำนวนเต็มปกติ<br>
                2) ตัวเลขหลังจุดให้อ่านเรียงตัวทีละตัว (ห้ามอ่านเป็นสิบหรือร้อย)<br>
                ...
                if op == "*":
                    ...
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (Step-by-step):</b><br>
                    เป้าหมายคือต้องทำให้ x อยู่คนเดียวโดดๆ ทางฝั่งซ้าย<br>
                    1) จากโจทย์ <span style='display:inline-flex; align-items:center;'>{f_html('x', str(a), '#2c3e50', is_bold=False)}</span> หมายความว่า x กำลังถูก "หาร" ด้วยเลข <b>{a}</b> เราจึงต้องกำจัดเลข {a} ที่เป็นตัวส่วนนี้ออกไป<br>
                    2) โดยใช้สมบัติการเท่ากัน: เราจะนำ <b>{a} มาคูณเข้าทั้งสองข้างของสมการ</b><br>
                    3) เขียนบรรทัดใหม่ได้ว่า: <span style='display:inline-flex; align-items:center;'>{f_html('x', str(a), '#2c3e50', is_bold=False)}</span> <b style='color:green;'>× {a}</b> = {x} <b style='color:green;'>× {a}</b><br>
                    4) ทางฝั่งซ้าย ตัวส่วน (ตัวหาร) {a} จะตัดกับตัวคูณ {a} หมดไป เหลือเพียงแค่ <b>x</b> ตัวเดียว<br>
...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังประมวลผลคำอธิบายและวาดภาพเฉลยแบบ Step-by-Step พร้อมเศษส่วนแนวตั้ง..."):
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ สร้างไฟล์สำเร็จ! (ล้างโจทย์เก่าออก 100% โค้ดจัดเต็มไม่มีการตัดทอน และปรับเศษส่วนเป็นแนวตั้งทุกจุดในเฉลยแล้วครับ!)")
    c1, c2 = st.columns(2)
    with c1:

...
def draw_beakers_svg(v1_l, v1_ml, v2_l, v2_ml):
    """วาดรูปถังตวงน้ำ 2 ถัง สำหรับโจทย์ปริมาตร"""
        ...
        while attempts < 300:
            ...
            if actual_sub_t == "ปริมาตรและความจุ (มิลลิลิตร ลิตร)":
                ...
                if q_cat == "compare":
                    ...
                    if op == "+":
                        q = f"{svg}จากรูป ถ้านำน้ำจากทั้งสองถังมาเทรวมกัน จะได้ปริมาตรน้ำทั้งหมดเท่าไร?"
                    else:
                        q = f"{svg}จากรูป ถัง A มีปริมาตรน้ำมากกว่าถัง B อยู่เท่าไร?"
                ...
                if q_cat == "compare":
                    ...
                    q = f"จงหาผลลัพธ์: <b>{v1_maj} {u_major} {v1_min} {u_minor} {op} {v2_maj} {u_major} {v2_min} {u_minor}</b>"
                    table_html, ans_str = generate_unit_math_html(u_major, u_minor, v1_maj, v1_min, v2_maj, v2_min, op, multiplier)
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (การตั้ง{'บวก' if op=='+' else 'ลบ'}แบบข้ามหน่วย):</b><br>
                    {table_html}
                    <b>ตอบ: {ans_str}</b></span>"""
...
if st.sidebar.button(f"{'🚀 สั่งสร้างข้อสอบระดับ Ultimate Challenge!' if is_challenge else '🚀 สั่งสร้างข้อสอบ King Math'}", type="primary", use_container_width=True):
    with st.spinner("กำลังตรวจสอบตรรกะระดับ Deep Scan และวาดภาพเศษส่วนแนวตั้ง..."):
        ...
        html_w = create_page(selected_grade, selected_sub, qs, is_key=False, q_margin=q_margin, ws_height=ws_height, brand_name=brand_name, is_challenge=is_challenge)
        html_k = create_page(selected_grade, selected_sub, qs, is_key=True, q_margin=q_margin, ws_height=ws_height, brand_name=brand_name, is_challenge=is_challenge)
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดอัปเดตเรียบร้อยครับ! รักษาสถานะโค้ด 100% แก้ไขบั๊กเรียบร้อย และเปลี่ยน Sidebar ให้ใช้หลักสูตรตรงตามภาพที่ส่งมาครับ")
    c1, c2 = st.columns(2)
    with c1:

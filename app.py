...
def draw_distance_route_svg(p_names, p_emojis, dist_texts):
    """วาดเส้นทางบอกระยะทางระหว่างสถานที่ (2 หรือ 3 สถานที่)"""
    width = 500
    height = 120
    svg = f'<div style="text-align:center; margin: 15px 0;"><svg width="{width}" height="{height}">'
    
    # วาดเส้นประเชื่อมต่อสถานที่
...
curriculum_db = {
    ...
    "ป.3": {
        ...
        "เวลา เงิน และการวัด": ["การบอกเวลาเป็นนาฬิกาและนาที", "การบอกจำนวนเงินทั้งหมด", "การอ่านน้ำหนักจากเครื่องชั่งสปริง", "การอ่านความยาวจากไม้บรรทัด", "ระยะทาง (กิโลเมตรและเมตร)"],
        ...
        while attempts < 300:
            ...
            if actual_sub_t == "ระยะทาง (กิโลเมตรและเมตร)":
                ...
                if q_type == "convert_to_km":
                    ...
                    # ถ้าเมตรตัวตั้งน้อยกว่าตัวลบ จะเกิดการยืม
                    if m1 >= m2:
                        m1, m2 = m2, m1 + 50
                        if m2 >= 1000: m2 = 950
                    ...
                    q = f"เส้นทางที่ 1 จาก <b>{p_names[0]}</b> ไป <b>{p_names[1]}</b> ระยะทาง {km1} กิโลเมตร {m1} เมตร<br>เส้นทางที่ 2 จาก <b>{p_names[0]}</b> ไป <b>{p_names[2]}</b> ระยะทาง {km2} กิโลเมตร {m2} เมตร<br>เส้นทางที่ 1 ไกลกว่าเส้นทางที่ 2 อยู่กี่กิโลเมตร กี่เมตร?"
                    sol = f"""<span style='color: #2c3e50;'><b>วิธีทำอย่างละเอียด (🔥 โหมดชาเลนจ์):</b><br>
                    นำระยะทางมาลบกัน โดยแยกตั้งหน่วยกิโลเมตรและเมตรให้ตรงกัน<br>
                    <b>ขั้นที่ 1: ลบหน่วยเมตร</b><br>
                    👉 ฝั่งตัวตั้งมี {m1} เมตร ซึ่งน้อยกว่าตัวลบ ({m2} เมตร) จึงลบไม่ได้<br>
                ...
                if grade == "ป.3":
                    # 📌 ป.3: การวัดทะลุ 1 เมตร และแปลงหน่วย เมตร เซนติเมตร มิลลิเมตร
                    if is_challenge:
                        # 🔥 โหมดชาเลนจ์: 2 สิ่งของ นำมาเปรียบเทียบผลต่าง
                        len_a = round(random.uniform(110.0, 350.0), 1)
                        len_b = round(random.uniform(110.0, 350.0), 1)
                        while abs(len_a - len_b) < 1.0: # ให้ต่างกันอย่างน้อย 1 ซม.
                            len_b = round(random.uniform(110.0, 350.0), 1)
                        ...
                        q = f"สายวัดด้านล่างแสดงตำแหน่งส่วนปลายของสิ่งของ 2 ชิ้น (เริ่มวัดจาก 0 เสมอ)<br>{svg_a}{svg_b}จงหาว่าสิ่งของใดมีความยาวมากกว่ากัน และมากกว่ากันอยู่เท่าไร? <br>(ตอบในหน่วย เมตร เซนติเมตร มิลลิเมตร)"
                        ...
                        👉 จะเห็นได้ชัดเจนว่า <b>{longer}</b> มีความยาวมากกว่า<br>
                        ...
                        <b>ตอบ: {longer} ยาวกว่าอยู่ {diff_str}</b></span>"""
                        
                    else:
                        # 📌 ป.3 ธรรมดา: สิ่งของเดียวแต่เกิน 1 เมตร
                        len_a = round(random.uniform(110.0, 350.0), 1)
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดอัปเดตเรียบร้อยครับ! มีเรื่องระยะทาง กิโลเมตร/เมตร พร้อมกราฟิกเส้นทางสวยงาม และความหลากหลายครบถ้วน")
    c1, c2 = st.columns(2)
    with c1:

...
<div class="main-header">
    ...
    <p>ระบบสร้างสื่อการสอนคณิตศาสตร์ (ป.1 - ป.6) พร้อมระบบ Branding, Spacing และโหมดทดสอบรวมสุ่มทุกเรื่อง</p>
</div>
""", unsafe_allow_html=True)
...
def generate_questions_logic(grade, main_t, sub_t, num_q):
    ...
    for _ in range(num_q):
        ...
        while attempts < 300:
            ...
            # ==============================
            # โหมดข้อสอบแข่งขัน (TMC) - อัปเกรดความยาก!
            # ==============================
            if actual_sub_t == "สัญลักษณ์และสมการ":
                val1 = random.randint(3, 9)
                val2 = random.randint(3, 9)
                while val1 == val2: val2 = random.randint(3, 9)
                val3 = random.randint(2, 9)
                while val3 == val1 or val3 == val2: val3 = random.randint(2, 9)
                
                eq1_ans = val1 * 3
                eq2_ans = val1 + val2
                eq3_ans = val2 + val3
                
                q = f"พิจารณาความสัมพันธ์ของรูปภาพต่อไปนี้:<br><br><span style='margin-left: 20px; font-weight: bold; color: #2c3e50;'>🍎 + 🍎 + 🍎 = {eq1_ans}</span><br><span style='margin-left: 20px; font-weight: bold; color: #2c3e50;'>🍎 + 🍌 = {eq2_ans}</span><br><span style='margin-left: 20px; font-weight: bold; color: #2c3e50;'>🍌 + 🍇 = {eq3_ans}</span><br><br>จงหาผลลัพธ์ของ <b>🍎 + 🍌 + 🍇</b> ว่ามีค่าเท่ากับเท่าใด?"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>1) จากบรรทัดแรก 🍎 3 ผลรวมกันได้ {eq1_ans} แสดงว่า 🍎 = {eq1_ans} ÷ 3 = <b>{val1}</b><br>2) นำ 🍎 ไปแทนค่าในบรรทัดที่สอง จะได้ {val1} + 🍌 = {eq2_ans} แสดงว่า 🍌 = {eq2_ans} - {val1} = <b>{val2}</b><br>3) นำ 🍌 ไปแทนค่าในบรรทัดที่สาม จะได้ {val2} + 🍇 = {eq3_ans} แสดงว่า 🍇 = {eq3_ans} - {val2} = <b>{val3}</b><br>ดังนั้น 🍎 + 🍌 + 🍇 = {val1} + {val2} + {val3} = </span><b>{val1 + val2 + val3}</b>"

            elif actual_sub_t == "โจทย์ปัญหาแถวคอย":
                front_pos = random.randint(6, 18)
                back_pos = random.randint(6, 18)
                total_people = front_pos + back_pos - 1
                girls = random.randint(3, total_people - 3)
                boys = total_people - girls
                names = ["ต้น", "กล้า", "ฟ้า", "พลอย", "บอย", "นนท์"]
                name = random.choice(names)
                
                q = f"เด็กกลุ่มหนึ่งยืนเข้าแถวตอนเรียงหนึ่ง <b>{name}</b>ยืนอยู่ลำดับที่ <b>{front_pos}</b> จากด้านหน้า และยืนอยู่ลำดับที่ <b>{back_pos}</b> จากด้านหลัง<br>หากในแถวนี้มีเด็กผู้หญิงทั้งหมด <b>{girls}</b> คน จะมีเด็กผู้ชายในแถวนี้กี่คน?"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>1) หาจำนวนคนทั้งหมดในแถวก่อน: นำลำดับจากข้างหน้า + ลำดับจากข้างหลัง แล้วต้อง <b>ลบ 1</b> (เพราะนับ{name}ซ้ำไป 2 ครั้ง)<br>จะได้คนทั้งหมด = {front_pos} + {back_pos} - 1 = <b>{total_people} คน</b><br>2) แถวนี้มีคนทั้งหมด {total_people} คน เป็นผู้หญิงไปแล้ว {girls} คน<br>ดังนั้น เป็นเด็กผู้ชาย = {total_people} - {girls} = </span><b>{boys} คน</b>"
                ...
                age_c = age_b - diff_c
                future = random.randint(4, 8) 
                
                current_sum = age_a + age_b + age_c
                future_sum = current_sum + (3 * future)
                
                q = f"ปัจจุบัน ครอบครัวหนึ่งมีพี่น้อง 3 คน คือ เอ บี และซี<br>- <b>เอ</b> มีอายุ <b>{age_a}</b> ปี<br>- <b>บี</b> มีอายุมากกว่าเอ <b>{diff_b}</b> ปี<br>- <b>ซี</b> มีอายุน้อยกว่าบี <b>{diff_c}</b> ปี<br>คำถาม: อีก <b>{future}</b> ปีข้างหน้า อายุของทั้งสามคนรวมกันจะเป็นกี่ปี?"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>1) หาอายุปัจจุบันของแต่ละคน:<br>เอ อายุ = {age_a} ปี<br>บี อายุ = {age_a} + {diff_b} = {age_b} ปี<br>ซี อายุ = {age_b} - {diff_c} = {age_c} ปี<br>อายุปัจจุบันรวมกัน = {age_a} + {age_b} + {age_c} = <b>{current_sum} ปี</b><br>2) ระวังโดนหลอก! อีก {future} ปีข้างหน้า ทั้ง 3 คนจะมีอายุเพิ่มขึ้น<b>คนละ</b> {future} ปี (รวมเพิ่มขึ้น 3 × {future} = <b>{3 * future} ปี</b>)<br>ดังนั้น ผลรวมอายุในอนาคต = {current_sum} + {3 * future} = </span><b>{future_sum} ปี</b>"

            elif actual_sub_t == "โจทย์ปัญหาสัตว์และขา":
                cows = random.randint(3, 12)
                chickens = random.randint(5, 15)
                total_heads = cows + chickens
                total_legs = (cows * 4) + (chickens * 2)
                
                q = f"ฟาร์มแห่งหนึ่งเลี้ยงวัวและไก่รวมกัน ถ้านับหัวของสัตว์ทั้งหมดจะได้ <b>{total_heads}</b> หัว และถ้านับขาของสัตว์ทั้งหมดรวมกันจะได้ <b>{total_legs}</b> ขา<br>อยากทราบว่าฟาร์มแห่งนี้เลี้ยง<b>วัว</b>กี่ตัว?"
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ (เทคนิคสมมติ):</b><br>1) สมมติให้สัตว์ทั้ง {total_heads} ตัว เป็นไก่ทั้งหมด (ไก่ 1 ตัว มี 2 ขา)<br>ควรจะมีขาทั้งหมด = {total_heads} × 2 = <b>{total_heads * 2} ขา</b><br>2) แต่โจทย์บอกว่ามี {total_legs} ขา แสดงว่ามีขาขาดไป = {total_legs} - {total_heads * 2} = <b>{total_legs - (total_heads * 2)} ขา</b><br>3) ขาที่ขาดไปคือส่วนของวัว (วัว 1 ตัว มีขามากกว่าไก่ 4 - 2 = 2 ขา)<br>ดังนั้น จำนวนวัว = ขาที่ขาดไป ÷ 2 = {total_legs - (total_heads * 2)} ÷ 2 = </span><b>{cows} ตัว</b>"
            ...
            # ==============================
                ...
                if random.choice([True, False]): 
                    ...
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> สังเกตจากป้ายเส้นชัยทางขวามือ แล้วนับย้อนมาทางซ้าย คันที่ 1, 2... จะพบว่ารถคันนี้อยู่ใน</span><br><b>อันดับที่ {idx + 1}</b> {ans_svg}"
                else: 
                    q = f"รถที่วิ่งอยู่ในอันดับที่ {idx + 1} คือรถสีอะไร? {svg_d}"
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> เริ่มนับคันแรกจากป้ายเส้นชัยฝั่งขวามือ นับย้อนไป {idx + 1} คัน จะพบว่าเป็น</span><br><b>สี{name}</b> {ans_svg}"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> สังเกตจะพบว่ารูปภาพมีการเรียงซ้ำกันเป็นชุด ชุดละ {len(set(pt))} รูป เมื่อนับลำดับต่อมาเรื่อยๆ รูปที่หายไปคือ:</span><br><br><svg width='30' height='30' style='vertical-align: middle;'>{shapes[seq[slen]]}</svg>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> <br>1. เข็มสั้น(สีแดง) มีเส้นประชี้บอกว่ายังอยู่ระหว่างเลข {h} กับ {h+1 if h<12 else 1} แสดงถึงชั่วโมงที่ {h} <br>2. เข็มยาว(สีน้ำเงิน) ชี้ที่ขีดนาทีที่ {m} (ดูตัวเลขด้านนอกประกอบ)<br>ถ้าเป็น{day} จึงอ่านเวลาได้</span> <b>{ans_h:02d}.{m:02d} น.</b>"
                ...
                sol = "<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> {explain} (จำไว้ว่าช่องเล็ก 1 ช่อง = 1 ขีด)<br>ดังนั้น สินค้ามีน้ำหนัก</span> <b>{ans_text}</b>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> นับรูปผลไม้ทั้งหมดรวมกันได้ {sum(counts)} รูปภาพ<br>และกำหนดให้ 1 รูปภาพ แทนผลไม้ {multiplier} ผล<br>นำ {sum(counts)} × {multiplier} =</span> <b>{sum(counts) * multiplier} ผล</b>"

            elif "การนับทีละ" in actual_sub_t:
                is_10 = "10" in actual_sub_t; is_1 = "1" in actual_sub_t; step = 10 if is_10 else (1 if is_1 else random.choice([2, 5, 10, 100]))
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> เปรียบเทียบค่าทีละหลักจากซ้ายไปขวา จะพบว่า {a:,} {comp_text} {b:,}<br>ตอบ: </span> <b>{sign}</b>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> แยกตัวเลขตามค่าประจำหลักจะได้:<br></span><b>{' + '.join(parts)}</b>"
                
            elif "จำนวนคู่" in actual_sub_t:
                n = random.randint(10, limit)
                q = f"จำนวน <b>{n:,}</b> เป็นจำนวนคู่ หรือ จำนวนคี่?"
                is_even = (n % 2 == 0)
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ดูที่หลักหน่วยคือเลข {n%10} ซึ่ง{'หารด้วย 2 ลงตัว' if is_even else 'หารด้วย 2 ไม่ลงตัว'}<br>ดังนั้นจึงเป็น</span> <b>{'จำนวนคู่' if is_even else 'จำนวนคี่'}</b>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ประมาณเป็นจำนวน{ptype} ให้พิจารณาตัวเลขใน {chk_p} ซึ่งก็คือเลข <b>{chk_d}</b><br>ตามกฎแล้วต้องทำการ <b>{action}</b> จะได้เป็น:</span> <b>{ans:,}</b>"
                ...
                q = f"จงเขียนเศษเกินต่อไปนี้ให้อยู่ในรูปจำนวนคละ : <span style='display:inline-flex; vertical-align: middle; margin-left: 10px;'>{frac_html}</span>"
                w, r = num // den, num % den; g = math.gcd(r, den)
                mixed_raw = generate_mixed_number_html(w, r, den)
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> นำตัวเศษตั้ง หารด้วยตัวส่วน<br>จะได้ <b>{num} ÷ {den} = {w} เศษ {r}</b><br>เขียนเป็นจำนวนคละโดยให้ <b>{w}</b> เป็นจำนวนเต็ม และ <b>{r}</b> เป็นเศษ: </span><br><br>{mixed_raw}"
                if g > 1:
                ...
                svg_bar = f'<div style="display: inline-block; vertical-align: middle; margin-left: 15px;"><svg width="{total_w + 4}" height="{bar_h + 4}"><g transform="translate(2,2)">{rects}</g></svg></div>'
                ...
                if grade == "ป.5":
                    ...
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ทำตัวส่วนให้เท่ากัน โดยหา ค.ร.น. ของ {d1} และ {d2} ซึ่งก็คือ <b>{lcm_d}</b></span><br><br>"
                    sol += f"<div style='display:flex; align-items:center; margin-bottom: 10px;'> {f1} <span style='margin:0 10px; font-weight: bold;'>{op}</span> {f2} <span style='margin:0 10px; font-weight: bold;'>=</span> {f1_ex} <span style='margin:0 10px; font-weight: bold;'>{op}</span> {f2_ex} </div>"
                    sol += f"<div style='display:flex; align-items:center; margin-bottom: 15px;'> <span style='margin:0 10px;'></span> <span style='margin:0 10px; font-weight: bold;'>=</span> {f1_new} <span style='margin:0 10px; font-weight: bold;'>{op}</span> {f2_new} <span style='margin:0 10px; font-weight: bold;'>=</span> {s_ans}</div>"
                    ...
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> เนื่องจากตัวส่วนเท่ากัน ให้นำตัวเศษมา{op}กันได้เลย</span><br><br><div style='display:flex; align-items:center; margin-bottom: 15px;'>{f1} <span style='margin:0 10px; font-weight: bold;'>{op}</span> {f2} <span style='margin:0 10px; font-weight: bold;'>=</span> {s1} <span style='margin:0 10px; font-weight: bold;'>=</span> {s2}</div>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ตั้งจุดทศนิยมให้ตรงกันแล้วดำเนินการตามปกติ</span><br>{sol_html}"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b><br>1) ตั้งคูณโดยนำจุดทศนิยมออกก่อน ({int_a} × {b}):</span><br>{vert_mul_html}<br><span style='color: #2c3e50;'>2) ตัวตั้งมีทศนิยม 1 ตำแหน่ง คำตอบจึงต้องใส่ทศนิยม 1 ตำแหน่ง<br>ตอบ:</span> <b>{ans:.1f}</b>"
                ...
                frac_html = generate_fraction_html(num, den)
                q = f"จงเขียนเศษส่วนต่อไปนี้ให้อยู่ในรูปร้อยละ : <span style='display:inline-flex; vertical-align: middle; margin-left: 10px;'>{frac_html}</span>"
                mul = 100 // den
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ทำตัวส่วนให้เป็น 100 โดยการคูณด้วย {mul} ทั้งเศษและส่วน</span><br><br><div style='display:flex; align-items:center;'>{frac_html} <span style='margin:0 10px;'>=</span> {generate_fraction_html(f'{num} × {mul}', f'{den} × {mul}')} <span style='margin:0 10px;'>=</span> {generate_fraction_html(num*mul, 100)}</div><br>ตอบ: <b>ร้อยละ {ans} หรือ {ans}%</b>"
                ...
                if random.choice([True, False]):
                    q = f"จงหาจำนวนที่แทนใน {box_html} แล้วทำให้ประโยคเป็นจริง : <span style='display:inline-flex; align-items:center; color: #3498db; font-weight: bold; margin-left: 10px;'>{a} : {b} = {c} : {box_html}</span>"
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> พิจารณาตัวหน้าจาก {a} ไปเป็น {c} พบว่าเกิดจากการคูณด้วย <b>{m}</b> (เพราะ {a} × {m} = {c})<br>ดังนั้น ตัวหลังที่หายไปต้องนำ {b} ไปคูณด้วย {m} ด้วยเช่นกัน<br>จะได้ {b} × {m} = </span><b>{d}</b>"
                else:
                    q = f"จงหาจำนวนที่แทนใน {box_html} แล้วทำให้ประโยคเป็นจริง : <span style='display:inline-flex; align-items:center; color: #3498db; font-weight: bold; margin-left: 10px;'>{a} : {b} = {box_html} : {d}</span>"
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> พิจารณาตัวหลังจาก {b} ไปเป็น {d} พบว่าเกิดจากการคูณด้วย <b>{m}</b> (เพราะ {b} × {m} = {d})<br>ดังนั้น ตัวหน้าที่หายไปต้องนำ {a} ไปคูณด้วย {m} ด้วยเช่นกัน<br>จะได้ {a} × {m} = </span><b>{c}</b>"
                ...
                if q_type == 1:
                    ...
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> อัตราส่วน {item1} : {item2} = {a} : {b}<br>เนื่องจากมี{item1} {val1} {u1} ซึ่งเกิดจากการนำ {a} ไปคูณด้วย <b>{m}</b> ({a} × {m} = {val1})<br>ดังนั้น ต้องนำอัตราส่วนของ{item2}ไปคูณด้วย {m} ด้วยเช่นกัน<br>จะได้ {b} × {m} = </span><b>{val2} {u2}</b>"
                else:
                    total = val1 + val2
                    ...
                    sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> อัตราส่วน {item1} : {item2} = {a} : {b}<br>นำอัตราส่วนมาบวกกันจะได้จำนวนส่วนทั้งหมด = {a} + {b} = <b>{a+b} ส่วน</b><br>ของทั้งหมด {total} {u1} แบ่งเป็น {a+b} ส่วน จะได้ส่วนละ {total} ÷ {a+b} = <b>{m} {u1}</b><br>ดังนั้น จำนวน{item1}มี {a} ส่วน จึงเท่ากับ {a} × {m} = </span><b>{val1} {u1}</b>"
                ...
                q = f"เสื้อราคา {price:,} บาท ร้านค้าลดราคาให้ {percent}% ร้านค้าลดราคาให้กี่บาท?<div style='margin-top: 10px; color: #2c3e50;'>{prefix} ...........................................................</div>"
                sol = f"<span style='color: #2c3e50;'><b>ประโยคสัญลักษณ์:</b> ({percent} ÷ 100) × {price:,} = {box_html}</span><br><br><span style='color: #2c3e50;'><b>วิธีทำ:</b><br>คำว่า ลดราคา {percent}% หมายถึง ({percent} ÷ 100) × ราคาป้าย<br>= ({percent} ÷ 100) × {price:,}<br>ตอบ:</span> <b>{discount:,} บาท</b>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> {explain}<br>ตอบ:</span> <b>{ans_type}</b>"

            elif "ความยาวรอบรูป" in actual_sub_t or ("พื้นที่" in actual_sub_t and "สี่เหลี่ยม" in actual_sub_t):
                is_peri = "ความยาวรอบรูป" in actual_sub_t; is_square = random.choice([True, False])
                if is_square:
                    s = random.randint(5, 30) if is_peri else random.randint(5, 25)
                    svg = f"""<div style="text-align: center; margin: 10px 0;"><svg width="150" height="150"><rect x="25" y="25" width="100" height="100" fill="#e8f4f8" stroke="#2980b9" stroke-width="3"/><line x1="70" y1="20" x2="80" y2="30" stroke="#c0392b" stroke-width="2"/><line x1="70" y1="120" x2="80" y2="130" stroke="#c0392b" stroke-width="2"/><line x1="20" y1="70" x2="30" y2="80" stroke="#c0392b" stroke-width="2"/><line x1="120" y1="70" x2="130" y2="80" stroke="#c0392b" stroke-width="2"/><text x="75" y="18" font-size="16" font-family="Sarabun" font-weight="bold" fill="#333" text-anchor="middle">{s} ซม.</text></svg></div>"""
                    q = f"จากรูปภาพ จงหา{'ความยาวรอบรูป' if is_peri else 'พื้นที่'}ของ<b>รูปสี่เหลี่ยมจัตุรัส</b> {svg}"
                    if is_peri: ans = 4 * s; sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ความยาวรอบรูปสี่เหลี่ยมจัตุรัส = 4 × ความยาวด้าน<br>= 4 × {s}<br>ตอบ:</span> <b>{ans} เซนติเมตร</b>"
                    else: ans = s * s; sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> พื้นที่สี่เหลี่ยมจัตุรัส = ด้าน × ด้าน<br>= {s} × {s}<br>ตอบ:</span> <b>{ans:,} ตารางเซนติเมตร</b>"
                else:
                    w = random.randint(5, 20); l = random.randint(w + 2, 30)
                    svg = f"""<div style="text-align: center; margin: 10px 0;"><svg width="220" height="120"><rect x="20" y="20" width="140" height="80" fill="#e8f8f5" stroke="#16a085" stroke-width="3"/><polyline points="20,35 35,35 35,20" fill="none" stroke="#16a085" stroke-width="1.5"/><polyline points="160,35 145,35 145,20" fill="none" stroke="#16a085" stroke-width="1.5"/><text x="90" y="15" font-size="16" font-family="Sarabun" font-weight="bold" fill="#333" text-anchor="middle">{l} ซม.</text><text x="165" y="65" font-size="16" font-family="Sarabun" font-weight="bold" fill="#333" text-anchor="start">{w} ซม.</text></svg></div>"""
                    q = f"จากรูปภาพ จงหา{'ความยาวรอบรูป' if is_peri else 'พื้นที่'}ของ<b>รูปสี่เหลี่ยมผืนผ้า</b> {svg}"
                    if is_peri: ans = 2 * (w + l); sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ความยาวรอบรูปสี่เหลี่ยมผืนผ้า = 2 × (กว้าง + ยาว)<br>= 2 × ({w} + {l})<br>= 2 × {w+l}<br>ตอบ:</span> <b>{ans} เซนติเมตร</b>"
                    else: ans = w * l; sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> พื้นที่สี่เหลี่ยมผืนผ้า = กว้าง × ยาว<br>= {w} × {l}<br>ตอบ:</span> <b>{ans:,} ตารางเซนติเมตร</b>"
                ...
                sol = f"<span style='color: #2c3e50;'><b>วิธีทำ:</b> ตั้งบวกหลักหน่วยและหลักสิบให้ตรงกัน</span><br><b>ตอบ: {a + b}</b>"
...
def create_page(grade, sub_t, questions, is_key=False, q_margin="10px", brand_name=""):
    ...
    <style>
        ...
        .workspace {{ height: 180px; border: 2px dashed #bdc3c7; border-radius: 8px; margin: 10px 0; padding: 10px; color: #95a5a6; font-size: 16px; display: flex; align-items: flex-start; }}
...
spacing_level = st.sidebar.select_slider("↕️ ระยะห่างระหว่างข้อ:", options=["แคบ", "ปานกลาง", "กว้าง"], value="แคบ")
q_margin = {"แคบ": "0px", "ปานกลาง": "10px", "กว้าง": "20px"}[spacing_level]
...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังประมวลผลลอจิกคณิตศาสตร์และวาดกราฟิก..."):
        ...
        html_w = create_page(selected_grade, selected_sub, qs, is_key=False, q_margin=q_margin, brand_name=brand_name)
        html_k = create_page(selected_grade, selected_sub, qs, is_key=True, q_margin=q_margin, brand_name=brand_name)
        ...
        <style>
            ...
            .title-box {{ margin-top: 80px; }} .title {{ font-size: 65px; color: #2c3e50; font-weight: margin: 0; }}

...
# 🎨 Inject Custom CSS ทำให้ดูเป็นมืออาชีพ
st.markdown("""
<style>
    ...
    /* ปรับระยะห่างของพื้นที่แสดงผลหลัก */
    .block-container {
        padding-top: 2rem;
        padding-bottom: 2rem;
        max-width: 1200px;
    ...
    /* แต่งปุ่ม "สร้างใบงาน" ใน Sidebar */
...
if 'ebook_html' in st.session_state:
    ...
    # แบ่ง 2 คอลัมน์สำหรับปุ่มดาวน์โหลด
    col_dl1, col_dl2 = st.columns(2)
    with col_dl1:
        st.markdown("### 🖨️ ดาวน์โหลด E-Book (คลิกเดียวจบ)")
        st.info("เปิดไฟล์นี้ในเบราว์เซอร์ แล้วกด **Ctrl+P** เพื่อพิมพ์เป็น PDF ได้เลย!")
        st.download_button(
            label="📄 โหลดไฟล์รวม (Full E-Book)",
    ...
    with col_dl2:
        st.markdown("### 🗂️ ดาวน์โหลดไฟล์แยกส่วน (Zip)")
        st.info("ได้ไฟล์แยกหน้าปก แบบฝึกหัด และเฉลย เพื่อนำไปจัดหน้าเอง")
        st.download_button(
            label="📥 โหลดไฟล์แยก (.zip)",
    ...
    st.info("👈 กรุณาเลือกตั้งค่าใบงานที่แถบเมนูด้านซ้าย แล้วกดปุ่ม สีเขียว 'สร้างใบงานเดี๋ยวนี้'")

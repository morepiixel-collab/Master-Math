...
if st.sidebar.button("🚀 สั่งสร้างใบงานเดี๋ยวนี้", type="primary", use_container_width=True):
    with st.spinner("กำลังออกแบบรูปภาพประกอบและวาดภาพเฉลยแบบ Step-by-Step..."):
        
        qs = generate_questions_logic(selected_grade, selected_main, selected_sub, num_input, False)
...
if 'ebook_html' in st.session_state:
    st.success(f"✅ โค้ดสมบูรณ์เต็มรูปแบบ! แก้ไขรูปภาพแผนภูมิ, นาฬิกา, และเครื่องชั่งเรียบร้อยแล้วครับ")
    c1, c2 = st.columns(2)
    with c1:

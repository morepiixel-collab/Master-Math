...
# ==========================================
st.set_page_config(page_title="Math Generator Pro", page_icon="🚀", layout="wide")
...
<style>
    ...
    div[data-testid="stSidebar"] div.stButton > button { background-color: #27ae60; color: white; border-radius: 8px; height: 3rem; font-size: 18px; font-weight: bold; transition: all 0.3s ease; border: none; }
    div[data-testid="stSidebar"] div.stButton > button:hover { background-color: #219653; box-shadow: 0 4px 8px rgba(0,0,0,0.1); transform: translateY(-2px); }
    ...
    .main-header { background: linear-gradient(135deg, #2980b9, #2c3e50); padding: 2rem; border-radius: 15px; color: white; margin-bottom: 2rem; box-shadow: 0 10px 20px rgba(0,0,0,0.1); }
...
<div class="main-header">
    <h1>🚀 Math Worksheet Generator Pro</h1>
    <p>ระบบสร้างสื่อการสอนและใบงานคณิตศาสตร์อัตโนมัติ (ระดับ ป.1 - ป.6) พร้อมเฉลยและหน้าปกสำหรับ E-Book</p>
</div>
""", unsafe_allow_html=True)
...
def generate_cover_html(grade, main_t, sub_t, num_q):
    ...
    <style>
        .cover-inner {{ width: 100%; height: 100%; padding: 40px; box-sizing: border-box; text-align: center; position: relative; border: 15px solid #3498db; background: white; }}
...
selected_main = st.sidebar.selectbox("📂 เลือกหัวข้อหลัก:", list(curriculum_db[selected_grade].keys()))
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 📚 ออปชันเสริม (E-Book)")
include_cover = st.sidebar.checkbox("🎨 สร้างหน้าปก (Cover Page)", value=True, help="เพิ่มหน้าปกสีสันสวยงามพร้อมบอกระดับชั้นและเรื่องอัตโนมัติ")
        ...
        <style>
            ...
            @media screen {{
                body {{ 
                    ...
                    background-color: #525659; /* สีพื้นหลังเหมือนโปรแกรม PDF */
                    display: flex; 
                ...
                .a4-wrapper {{ 
                    width: 210mm; 
                    min-height: 297mm; /* ขนาด A4 แนวตั้ง */
                    background: white; 
                    margin-bottom: 30px; 
                ...
                .cover-wrapper {{
                    padding: 0; /* หน้าปกไม่เอา margin กระดาษ */
                }}
            }}
            ...
            @media print {{
                ...
                .cover-wrapper {{ height: 260mm; }} /* ให้พอดีขอบในตอนปริ้นท์ */
            }}
...
if 'ebook_html' in st.session_state:
    ...
    # ใช้งาน Column เพื่อแยกหมวดหมู่ปุ่ม
    ...
    # ใช้ streamlit.components.v1 เพื่อ render HTML สดๆ ในหน้าเว็บ
